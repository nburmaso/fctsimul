...
auto FCTGeometry::GeantDetector::constructDetector(const Config& cfg,
                                                   const Acts::Logger& logger)
  -> std::tuple<DetectorPtr, ContextDecorators, DetectorElements>
{
  if (cfg.g4World == nullptr) {
    throw std::invalid_argument("Detector: no world Geant4 volume provided");
  }
...
auto FCTGeometry::GeantDetector::constructTrackingGeometry(
...
auto FCTGeometry::GeantDetector::convertGeant4Volumes(
  const Config& cfg, const Acts::Logger& logger) const
  -> std::tuple<Surfaces, DetectorElements>
{
